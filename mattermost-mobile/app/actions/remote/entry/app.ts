...
export async function appEntry(serverUrl: string, since = 0) {
    const resolvedUrl = DatabaseManager.resolveServerUrlKey(serverUrl) || serverUrl;
    const operator = DatabaseManager.serverDatabases[resolvedUrl]?.operator;
    if (!operator) {
    ...
    if (!since) {
        if (Object.keys(DatabaseManager.serverDatabases).length === 1) {
            await setLastServerVersionCheck(resolvedUrl, true);
        }
    }
