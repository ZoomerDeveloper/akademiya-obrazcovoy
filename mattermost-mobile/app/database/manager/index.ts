...
// iOS: opening app.db then server DB(s) with the JSI SQLite driver can deadlock during cold start
// (JS never reaches Navigation.setRoot → splash never dismisses). Bridge mode is slower but stable.
const USE_WATERMELON_JSI_SQLITE = Platform.OS !== 'ios';
...
class DatabaseManagerSingleton {
    ...
    constructor() {
        ...
        this.databaseDirectory = this.getDatabaseDirectory();
    }

    private getDatabaseDirectory = (): string => {
        const fallbackDirectory = `${documentDirectory || ''}databases/`;

        if (Platform.OS !== 'ios') {
            return fallbackDirectory;
        }

        const {appGroupDatabase} = getIOSAppGroupDetails();
        if (appGroupDatabase) {
            return appGroupDatabase;
        }

        logDebug('App Group database path unavailable on iOS, falling back to document directory');
        return fallbackDirectory;
    };

    private ensureDatabaseDirectory = async (): Promise<void> => {
        if (!this.databaseDirectory) {
            throw new Error('Database directory is not configured');
        }

        if (!this.databaseDirectory.startsWith('file://')) {
            return;
        }

        const directoryInfo = await getInfoAsync(this.databaseDirectory);
    ...
    */
    public init = async (serverUrls: string[]): Promise<void> => {
        ...
        this.initPromise = this.initializeDatabase(serverUrls).
            then(() => {
    ...
    /**
     * ensureInitialized: Waits until the app database is usable (and full init finished when required).
     * Call this before accessing the database in async contexts.
     * @returns {Promise<void>}
     */
    public ensureInitialized = async (): Promise<void> => {
        ...
        // App DB is created mid-init; awaiting initPromise here deadlocks callers inside initializeDatabase
        // (e.g. beforeUpgrade → getLastInstalledVersion → ensureInitialized).
        if (this.appDatabase) {
            return;
        }
    ...
    private initializeDatabase = async (serverUrls: string[]): Promise<void> => {
        const appDatabase = await this.createAppDatabase();
        if (!appDatabase) {
            throw new Error('App database initialization failed');
        }
        ...
        if (__DEV__) {
            logDebug('[MM] beforeUpgrade done');
        }
        for await (const serverUrl of serverUrls) {
            await this.initServerDatabase(serverUrl);
        }
        if (__DEV__) {
            logDebug(`[MM] server DBs opened (${serverUrls.length} url(s))`);
        }
        ...
        if (__DEV__) {
            logDebug('[MM] DatabaseManager.initializeDatabase finished');
        }
    };
    ...
    private createAppDatabase = async (): Promise<AppDatabase|undefined> => {
        try {
            ...
            const adapter = new SQLiteAdapter({
                ...
                jsi: USE_WATERMELON_JSI_SQLITE,
                schema,
            });

            // Must complete native initialize/setUp before Database builds collections (otherwise iOS async
            // bridge can stall after "Opened database" and never resolve the init promise).
            await adapter.initializingPromise;
            logDebug('App SQLite adapter initialized');
    ...
    */
    ...
    public getAppDatabaseAndOperator = () => {
        ...
        if (!app) {
            if (!this.isInitialized) {
                throw new Error('Database not yet initialized. Please wait for DatabaseManager.init() to complete.');
            }
    ...
    */
    private getDatabaseFilePath = (dbName: string): string => {
        const normalizedDirectory = this.databaseDirectory.endsWith('/') ? this.databaseDirectory.slice(0, -1) : this.databaseDirectory;
        return `${normalizedDirectory}/${dbName}.db`;
    };
    ...
    /**
     * Canonical key in serverDatabases for a URL that may differ by protocol/trailing slash
     * from the row stored in the app database.
     */
    public resolveServerUrlKey = (serverUrl: string): string | undefined => {
        if (this.serverDatabases[serverUrl]) {
            return serverUrl;
        }
        const found = this.searchUrl(serverUrl);
        if (found && this.serverDatabases[found]) {
            return found;
        }
        return undefined;
    };
