...
/** Согласие на обработку ПДн (экран Академии). */
export const getPrivacyConsentAccepted = async (): Promise<boolean> => {
    try {
        await DatabaseManager.ensureInitialized();
        const {database} = DatabaseManager.getAppDatabaseAndOperator();
        const row = await database.get<GlobalModel>(GLOBAL).find(GLOBAL_IDENTIFIERS.PRIVACY_CONSENT);
