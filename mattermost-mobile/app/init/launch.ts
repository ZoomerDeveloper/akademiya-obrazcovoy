...
import {promptPrivacyConsentAfterHomeIfNeeded, resetToHome, resetToSelectServer, resetToTeams, resetToOnboarding} from '@screens/navigation';
...
export const launchToHome = async (props: LaunchProps) => {
    ...
    let nTeams = 0;
    if (props.serverUrl) {
        const dbKey = DatabaseManager.resolveServerUrlKey(props.serverUrl) || props.serverUrl;
        const database = DatabaseManager.serverDatabases[dbKey]?.database;
        if (database) {
    ...
    if (nTeams) {
        ...
        const rootResult = resetToHome(props);
        await Promise.resolve(rootResult);
        promptPrivacyConsentAfterHomeIfNeeded();
        return rootResult;
    }
