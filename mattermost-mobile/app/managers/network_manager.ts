...
/** Avoid blocking app launch forever when the server is unreachable (splash screen never dismisses). */
const CREATE_CLIENT_INIT_TIMEOUT_MS = 20000;
            ...
            try {
                await Promise.race([
                    ...
                    new Promise<never>((_, reject) => {
                        setTimeout(() => {
                            reject(new Error(`createClient timed out after ${CREATE_CLIENT_INIT_TIMEOUT_MS}ms (${serverUrl})`));
                        }, CREATE_CLIENT_INIT_TIMEOUT_MS);
                    }),
                ]);
            } catch (error) {
                logWarning('NetworkManager init skipped or failed for server', serverUrl, error);
            }
        }
    };
