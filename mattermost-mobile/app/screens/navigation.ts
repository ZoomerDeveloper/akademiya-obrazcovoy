...
import {Appearance, DeviceEventEmitter, InteractionManager, Platform, Alert, type EmitterSubscription, Keyboard, StatusBar} from 'react-native';
...
/** Оверлей согласия на ПДн (ТЗ Академии). Стабильный id — для dismissOverlay. */
export function showPrivacyConsentOverlay() {
    showOverlay(
        Screens.PRIVACY_CONSENT,
        {},
        {overlay: {interceptTouchOutside: true}},
        Screens.PRIVACY_CONSENT,
    );
...
/** После setRoot Home: показать согласие, если ещё не принято. */
export function promptPrivacyConsentAfterHomeIfNeeded() {
